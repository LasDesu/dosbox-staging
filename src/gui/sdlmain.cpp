...
static bool LoadGLShaders(const std::string_view source_sv, GLuint *vertex,
                          GLuint *fragment)
{
	if (source_sv.empty() || !vertex || !fragment)
		return false;
		...
		if (sdl.opengl.use_shader) {
			...
			if (glGetError()==GL_NO_ERROR && (sdl.opengl.program_object==0 || prog!=sdl.opengl.program_object)) {
				...
				if (sdl.opengl.program_object == 0) {
					...
					if (!LoadGLShaders(sdl.opengl.shader_sv,
					                   &vertexShader,
					                   &fragmentShader)) {
...
{
#if C_OPENGL
	if (!sdl.opengl.use_shader || sdl.opengl.shader_sv == source)
		return;

	sdl.opengl.shader_sv = source;
	if (sdl.opengl.program_object) {
		glDeleteProgram(sdl.opengl.program_object);
		sdl.opengl.program_object = 0;
	}
...
static void ListGlShaders()
{
	...
	for (const auto &line : RENDER_InventoryShaders())
		printf("%s\n", line.c_str());
#else
	printf("OpenGL is not available, so not listing shaders.\n");
#endif
}
